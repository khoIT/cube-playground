/**
 * merge-on-date-value (frontend) — mirror of
 * chat-service/src/tools/merge-on-date-value.ts.
 *
 * Aligns two independently-loaded Cube result sets onto a single date axis so
 * they render as one dual-axis chart. The two series come from cubes with no
 * Cube join, so their date members differ ("active_daily.log_date" vs
 * "user_recharge_daily.log_date") and keying on the cube-prefixed member never
 * overlaps. We project each row's date onto a synthetic `__date` column (the
 * date VALUE, identical across cubes at the same grain) and full-outer join over
 * the union of dates — a date present in only one series keeps its row with the
 * missing side absent (renders as a gap), never dropped.
 *
 * Measure columns keep their full Cube member ref as the row key so the chart's
 * column descriptors can resolve labels/units.
 */
