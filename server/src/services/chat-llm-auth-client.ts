...
/**
 * Switch the auth mode. ok=false with errorMessage when chat-service rejects
 * the mode (400 — lane not configured); ok=false without a message on
...
export async function setLlmAuthMode(
  ...
  try {
    ...
    if (res.status === 400) {
      ...
      return { ok: false, errorMessage: body.message ?? body.error ?? 'Mode rejected by chat-service' };
    }
