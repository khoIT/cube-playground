...
import {
  ...
  physicalizeQuery,
  logicalizeRows,
} from '../src/services/cube-member-resolver.js';

describe('cube-member-resolver', () => {
  describe('null prefix is a strict no-op (game_id / local)', () => {
    it('returns members, cubes, queries, rows unchanged', () => {
      ...
      expect(logicalCube('mf_users', null)).toBe('mf_users');
      const q = { measures: ['mf_users.user_count'], filters: [{ member: 'mf_users.country', operator: 'equals', values: ['VN'] }] };
  ...
  describe('physicalMember', () => {
    it('prefixes the cube part', () => {
    ...
    it('does not skip a cube that merely starts with the prefix string but not the _ boundary', () => {
      // prefix 'cfm', unrelated cube 'cfmx_users' → must still be prefixed
      expect(physicalMember('cfmx_users.x', 'cfm')).toBe('cfm_cfmx_users.x');
    });
  ...
  describe('physicalCube / logicalCube', () => {
    it('prefixes / strips a bare cube name idempotently', () => {
      expect(physicalCube('active_daily', 'ballistar')).toBe('ballistar_active_daily');
      expect(physicalCube('ballistar_active_daily', 'ballistar')).toBe('ballistar_active_daily');
      expect(physicalCube('active_daily', null)).toBe('active_daily');
    });
    it('strips the prefix', () => {
      expect(logicalMember('ballistar_mf_users.user_count', 'ballistar')).toBe('mf_users.user_count');
      expect(logicalCube('ballistar_mf_users', 'ballistar')).toBe('mf_users');
    });
    it('no-ops when not prefixed', () => {
      expect(logicalMember('mf_users.user_count', 'ballistar')).toBe('mf_users.user_count');
      expect(logicalCube('mf_users', 'ballistar')).toBe('mf_users');
    });
  });
  ...
  describe('physicalizeQuery', () => {
    it('rewrites measures, dimensions, timeDimensions, filters (nested), order, segments', () => {
      const q = {
        ...
        order: { 'mf_users.user_count': 'desc' as const },
    ...
    it('leaves already-physical slice filters untouched (idempotent)', () => {
      const q = {
        measures: ['mf_users.user_count'],
