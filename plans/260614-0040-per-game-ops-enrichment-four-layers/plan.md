...
| 8 | [Tests + pre-aggs + validation](phase-08-tests-preaggs-validation.md) | PARTIAL | 7 | DONE: vitest (member360 data-layer, redaction, ops-tab contract); `billing_detail` big-cube scan guard in `cube.js` (extracted to `behavior-bounds-guard.cjs` + 7 unit tests); CubeStore `billing_detail` rollup BUILT + sealed cfm+jus, routing verified via COMPILED-SQL FROM clause (`preagg_<game>.billing_detail_billing_detail_daily_batch`, serves real June data). REMAINING: ground-truth WITH real_users_only; playwright; deploy/rollback (push to `second` → auto-deploy). |
