/* global React, Icon, Button, Badge, Input, Tooltip, Popover, Card, Sparkline, Avatar, Switch,
   ...
   CONCEPTS, CONCEPT_BY_ID, OWNERS, useNav, useToast, Modal, Divider, FeedbackWidget */
...
const ExplorePage = ({ initialMeasureId, initialDimensionId, tweaks, openSaveView, openChangeAnalysis }) => {
  ...
  const [measureIds, setMeasureIds] = useStateE(() => [initialMeasureId || "revenue.total_vnd"]);
  ...
  const measures = measureIds.map(id => CONCEPT_BY_ID[id]).filter(Boolean);
  const dimensions = dimensionIds.map(id => CONCEPT_BY_ID[id]).filter(Boolean);
  const segments = segmentIds.map(id => CONCEPT_BY_ID[id]).filter(Boolean);
    ...
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", minWidth: 0, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "10px 20px", display: "flex", alignItems: "center", gap: 8, background: "var(--neutral-50)", borderBottom: "1px solid var(--border)" }}>
          ...
          {dimensions.some(d => d.id === "dim.day") && (
        ...
        <VerbChipBar
          ...
          onAdd={(action) => {
            if (action.type === "by_dim") {
              ...
              if (action.dimId === "dim.day") setChart("line");
              toast?.(`Sliced by ${CONCEPT_BY_ID[action.dimId]?.label}`, { icon: "rows-3" });
            }
            if (action.type === "compare") { setComparison(action.label); toast?.(`Comparing ${action.label}`, { icon: "git-compare" }); }
            if (action.type === "filter") { setSegmentIds(ids => ids.includes(action.segId) ? ids : [...ids, action.segId]); toast?.(`Filtered to ${CONCEPT_BY_ID[action.segId]?.label}`, { icon: "filter" }); }
...
const VerbChipBar = ({ measures, dimensions, segments, onAdd, chipPlacement }) => {
  ...
  const dimSuggestions = ["dim.country", "dim.channel", "dim.platform", "dim.payer_tier", "dim.day"]
    .filter(id => !dimensions.find(d => d.id === id))
    .map(id => CONCEPT_BY_ID[id]).filter(Boolean);
  const segSuggestions = ["seg.whales", "seg.vn_only", "seg.lapsed_payer_14d"]
...
function generateRows(measures, dimensions, segments, period) {
  ...
  const valueLists = {
    "dim.country": ["VN", "TH", "ID", "PH", "MY", "SG", "VND_other"],
    "dim.channel": ["organic", "facebook_ads", "google_ads", "tiktok", "cross_promo", "influencer"],
    "dim.platform": ["ios", "android", "web"],
    "dim.payer_tier": ["whale", "dolphin", "minnow", "non_payer"],
    "dim.game": ["ballistar", "league_of_thieves", "speed_lane"],
    "dim.day": Array.from({ length: 14 }).map((_, i) => `May ${(6 + i).toString().padStart(2, "0")}`),
  };
