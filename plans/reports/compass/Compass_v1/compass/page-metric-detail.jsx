/* global React, Icon, Button, Badge, Input, Tabs, Tooltip, Popover, Card, Avatar, Switch, Sparkline,
   ...
   FeedbackWidget, UsageChip, OwnerStamp, Metric,
   CONCEPTS, CONCEPT_BY_ID, OWNERS, ACTIVITY, LINEAGE, CHANGE_ANALYSIS,
   useNav, useToast, Modal, SectionHeader, Divider */
/* Compass — Metric Detail page. Implements all six panels per PRD §5.2. */

const { useState: useStateMD, useMemo: useMemoMD } = React;

const MetricDetailPage = ({ id, tweaks, openSubscribe, openChangeAnalysis, openSaveView }) => {
  const { go } = useNav();
  const toast = useToast();
  const concept = CONCEPT_BY_ID[id] || CONCEPTS[0];
  const [editMode, setEditMode] = useStateMD(false);
  const [editing, setEditing] = useStateMD(null); // which field key is currently being edited
  ...
  const isAuthor = true; // hard-wire to author for demo
        ...
        <div style={{ maxWidth: 1080, margin: "0 auto", padding: "24px 28px 60px" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 16 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 6, color: "var(--neutral-500)", fontSize: 13 }}>
                <TypeChip type={concept.type} />
                <span style={{ fontFamily: "var(--font-mono)" }}>{concept.cube}.{concept.member}</span>
                {concept.parameter && <span style={{ fontFamily: "var(--font-mono)" }}>(n={paramN})</span>}
          ...
          {tab === "overview" && (
            <OverviewTab concept={concept} draft={draft} setDraft={setDraft} editMode={editMode} editing={editing} setEditing={setEditing} paramN={paramN} setParamN={setParamN} go={go} />
          )}
          {tab === "formula" && (
            <FormulaTab concept={concept} draft={draft} setDraft={setDraft} editMode={editMode} editing={editing} setEditing={setEditing} />
          )}
...
// ─── OVERVIEW TAB ──────────────────────────────────────────────────
const OverviewTab = ({ concept, draft, setDraft, editMode, editing, setEditing, paramN, setParamN, go }) => (
...
// ─── FORMULA TAB ───────────────────────────────────────────────────
const FormulaTab = ({ concept, draft, setDraft, editMode, editing, setEditing }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
    <Panel title="Business formula" icon="calculator" description="Plain-English definition. This is what shows up in catalog search and digest narratives." badge={editMode && "editable"}>
      ...
      {concept.composedOf && (
        <div style={{ marginTop: 10, padding: 10, border: "1px dashed var(--border)", borderRadius: 8, fontSize: 12, color: "var(--neutral-600)" }}>
          <b>Composed of:</b> {concept.composedOf.map(id => CONCEPT_BY_ID[id]?.label || id).join(" · ")}
        </div>
      )}
    </Panel>
...
const SlicesTab = ({ concept, go, openSaveView }) => {
  const slices = (concept.sliceable || []).map(s => CONCEPTS.find(c => c.id === "dim." + s) || { id: s, label: s.replace(/_/g, " "), type: "dimension" });
  const segs   = (concept.joinableSegments || []).map(s => CONCEPTS.find(c => c.id === "seg." + s)).filter(Boolean);
  const sims   = (concept.similar || []).map(id => CONCEPT_BY_ID[id]).filter(Boolean);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 8 }}>
          {slices.map(s => (
            <div key={s.id} onClick={() => go({ name: "explore", measureId: concept.id, dimensionId: s.id })} style={{
              ...
              <div style={{ flex: 1, fontSize: 13, fontWeight: 500, color: "var(--neutral-900)" }}>By {s.label}</div>
      ...
      <Panel title="Joinable segments" icon="filter" description="Segments that filter the user population this measure runs over.">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 8 }}>
          {segs.map(s => (
            <div key={s.id} onClick={() => go({ name: "metric", id: s.id })} style={{
              display: "flex", alignItems: "center", gap: 10, padding: "10px 12px",
              border: "1px solid var(--border)", borderRadius: 8, cursor: "pointer", background: "#fff",
      ...
      <Panel title="Similar measures" icon="copy" description="Related concepts you might also want to compare or use instead.">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 8 }}>
          {sims.map(s => (
            <ConceptCard key={s.id} concept={s} variant="list" onClick={() => go({ name: "metric", id: s.id })} />
          ))}
        </div>
      </Panel>
    </div>
