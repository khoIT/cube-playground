/* global React, Icon, Button, Badge, Input, Card, Tooltip, Switch, Sparkline, Avatar, Modal, Tabs, Divider,
   ...
   CONCEPTS, METRICS, CATALOG_BY_ID, OWNERS, NOTIFICATIONS, SAVED_VIEWS, CHANGE_ANALYSIS,
   useNav, useToast */
  ...
  return (
    <div style={{ maxWidth: 1240, margin: "0 auto", padding: "24px 28px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 12 }}>
        {SAVED_VIEWS.map(v => {
          const m = CATALOG_BY_ID[v.measures[0]];
          return (
            <div key={v.id} onClick={() => go({ name: "explore", measureId: v.measures[0], dimensionId: v.dimensions[0] })} style={{
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                {v.measures.map(id => { const c = CATALOG_BY_ID[id]; return c && <Badge key={id} variant="outline" leftIcon="sigma">{c.label}</Badge>; })}
                {v.dimensions.map(id => { const c = CATALOG_BY_ID[id]; return c && <Badge key={id} variant="outline" leftIcon="rows-3">by {c.label}</Badge>; })}
                {v.filters.map(id => { const c = CATALOG_BY_ID[id]; return c && <Badge key={id} variant="outline" leftIcon="filter">{c.label}</Badge>; })}
...
const DigestPage = () => {
  ...
  const subscriptions = [
    { id: 1, conceptId: "m.revenue",  cadence: "weekly Monday 09:00", channel: "slack", changeOnly: false },
    { id: 2, conceptId: "m.dau",          cadence: "daily 09:00", channel: "slack + email", changeOnly: false },
    { id: 3, conceptId: "m.npu",       cadence: "on change only", channel: "slack", changeOnly: true },
    { id: 4, conceptId: "m.transactions", cadence: "on change only", channel: "slack", changeOnly: true },
    { id: 5, conceptId: "m.paying_users",   cadence: "weekly Monday 09:00", channel: "email", changeOnly: false },
  ];
        ...
        <div>
          <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
            ...
            {subscriptions.map(sub => {
              const c = CATALOG_BY_ID[sub.conceptId];
              if (!c) return null;
              return (
...
const SlackDigestPreview = () => (
  <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: 12, padding: 14, fontFamily: "var(--font-sans)" }}>
    ...
    <SlackBlock>
      <SlackHeader concept={CATALOG_BY_ID["m.npu"]} anomaly />
      <div style={{ fontSize: 12, color: "var(--neutral-700)", lineHeight: 1.55, marginTop: 6 }}>
        NPU up <b>+8.7%</b> WoW. New paying users in <b>VN · organic</b> drove most of the lift — <b>+12.4%</b> in that slice alone.
      </div>
      <SlackButtons />
    </SlackBlock>
    <SlackBlock>
      <SlackHeader concept={CATALOG_BY_ID["m.revenue"]} />
      <div style={{ fontSize: 12, color: "var(--neutral-700)", lineHeight: 1.55, marginTop: 6 }}>
        Revenue <b>581.2M VND</b> · <b style={{ color: "var(--success)" }}>+4.2%</b> WoW. Whales in VN drove most of the lift.
      </div>
      <SlackButtons />
    </SlackBlock>
    <SlackBlock>
      <SlackHeader concept={CATALOG_BY_ID["m.arppu"]} />
      <div style={{ fontSize: 12, color: "var(--neutral-700)", lineHeight: 1.55, marginTop: 6 }}>
        ARPPU <b>65,980 VND</b> · <b style={{ color: "var(--success)" }}>+1.2%</b> WoW. Holding steady — worth checking the slope on payer tier.
      </div>
      <SlackButtons />
    </SlackBlock>
  </div>
...
const EmailDigestPreview = () => (
  <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: 12, padding: 0, overflow: "hidden", fontFamily: "var(--font-sans)" }}>
    ...
    <div style={{ padding: "18px 20px" }}>
      ...
      {["m.transactions", "m.revenue", "m.npu"].map(id => {
...
const NotificationsPage = ({ openChangeAnalysis }) => {
  ...
  return (
    <div style={{ maxWidth: 880, margin: "0 auto", padding: "24px 28px" }}>
      ...
      <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
        {NOTIFICATIONS.map(n => {
          const c = n.concept ? CATALOG_BY_ID[n.concept] : null;
          return (
...
const WorkspacesPage = () => {
  const ws = [
    { id: 1, name: "VN Ops weekly", owner: "minh", description: "Lapsed payers, whale revenue, refund rate.", metrics: ["m.revenue", "m.transactions", "bb.seg.lapsed_payer_14d"], shared: "team" },
    { id: 2, name: "Exec MTD", owner: "hieu", description: "Revenue MTD, ARPPU, paying users.", metrics: ["m.revenue", "m.arppu", "m.paying_users"], shared: "org" },
    { id: 3, name: "UA dashboard", owner: "tuan", description: "Installs, CPI, ROAS D7.", metrics: ["m.nru", "m.arpnpu", "m.revnpu"], shared: "team" },
  ];
  return (
    <div style={{ maxWidth: 1240, margin: "0 auto", padding: "24px 28px" }}>
      <div style={{ display: "flex", alignItems: "flex-start", marginBottom: 18 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(360px, 1fr))", gap: 12 }}>
        {ws.map(w => (
          <Card key={w.id} hover padding={20} asLink>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
              {w.metrics.map(id => { const c = CATALOG_BY_ID[id]; return c && <Badge key={id} variant="outline" leftIcon={c.type === "measure" ? "sigma" : c.type === "segment" ? "filter" : "rows-3"}>{c.label}</Badge>; })}
            </div>
          </Card>
        ))}
...
const ChangeAnalysisModal = ({ open, onClose, conceptId = "m.revenue" }) => {
  const data = CHANGE_ANALYSIS[conceptId];
  const c = CATALOG_BY_ID[conceptId];
