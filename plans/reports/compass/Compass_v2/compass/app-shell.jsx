/* global React, Icon, Button, Badge, Avatar, Input, Tooltip, Popover, Kbd, ConceptCard, MetricCard, CONCEPTS, METRICS, CATALOG_BY_ID, NOTIFICATIONS, TypeIcon */
...
// Searches METRICS (consumer-first) AND CONCEPTS (building blocks) — returns
// a mixed list ranked by score with metric-layer items lightly boosted so the
// consumer-friendly result ranks above its underlying building block.
const searchConcepts = (q) => {
  if (!q.trim()) return [];
  const ql = q.toLowerCase();
  const pool = [...METRICS, ...CONCEPTS];
  const scored = pool.map(c => {
    let score = 0;
    if (c.label.toLowerCase().includes(ql)) score += 10;
    if (c.label.toLowerCase().startsWith(ql)) score += 8;
    if ((c.standFor || "").toLowerCase().includes(ql)) score += 8;
    if ((c.synonyms || []).some(s => s.toLowerCase().includes(ql))) score += 7;
    if ((c.description || "").toLowerCase().includes(ql)) score += 3;
    if (c.cube && `${c.cube}.${c.member}`.includes(ql)) score += 5;
    if ((c.sampleQuestions || []).some(s => s.toLowerCase().includes(ql))) score += 2;
    if (score > 0 && c.type === "metric") score += 1.5;  // gentle bias toward metric-layer
    return { c, score };
        ...
        <div style={{ maxHeight: 460, overflowY: "auto", padding: 8 }}>
          {!q && (
            <div style={{ padding: 12 }}>
              ...
              <div style={{ marginTop: 16, fontSize: 11, fontWeight: 600, color: "var(--neutral-500)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>Popular metrics</div>
              {METRICS.slice(0, 3).map(m => (
                <SearchResultRow key={m.id} item={m} onClick={() => { go({ name: "metric", id: m.id }); onClose(); }} />
              ))}
            </div>
          )}
          {q && results.length === 0 && (
            <div style={{ padding: 24, textAlign: "center" }}>
              <div style={{ fontSize: 13, color: "var(--neutral-600)", marginBottom: 6 }}>Nothing matched <b style={{ color: "var(--neutral-900)" }}>"{q}"</b></div>
          ...
          {q && results.length > 0 && (
            <div>
              <div style={{ padding: "6px 12px 4px", fontSize: 11, fontWeight: 600, color: "var(--neutral-500)", textTransform: "uppercase", letterSpacing: "0.06em" }}>{results.length} matches · metric layer ranked above building blocks</div>
              {results.map(c => (
                <SearchResultRow key={c.id} item={c} onClick={() => {
                  if (c.type === "metric") go({ name: "metric", id: c.id });
                  else go({ name: "data-model", id: c.id });
                  onClose();
                }} />
              ))}
...
// Compact result row used inside the global search overlay. Renders either
// a metric-layer item (with the ƒ glyph + tier badge) or a building block.
const SearchResultRow = ({ item, onClick }) => {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      {isMetric ? (
        <span style={{
          width: 24, height: 24, borderRadius: 6, background: "var(--type-metric-bg)",
          border: "1px solid var(--border)", display: "inline-flex", alignItems: "center", justifyContent: "center",
          fontFamily: "var(--num-font)", fontWeight: 700, fontSize: 12, color: "var(--neutral-950)", flexShrink: 0,
        }}>ƒ</span>
      ) : (
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
          <span style={{ fontFamily: "var(--font-sans)", fontWeight: 500, fontSize: 13, color: "var(--neutral-950)" }}>{item.label}</span>
          {isMetric
            ? <span style={{ fontSize: 11, color: "var(--neutral-500)" }}>{item.standFor}</span>
            : <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--neutral-500)" }}>{item.cube}.{item.member}</span>}
          {item.trust === "certified" && <span style={{ color: "var(--trust-certified)" }}><Icon name="shield-check" size={11} /></span>}
        </div>
        <div style={{ fontSize: 12, color: "var(--neutral-500)", marginTop: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {isMetric ? (item.formula?.plain || item.description) : item.description}
        </div>
      </div>
      <span style={{
        flexShrink: 0, padding: "1px 7px", borderRadius: 4, fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em",
        color: isMetric ? "var(--layer-metric-accent)" : "var(--layer-data-accent)",
        background: isMetric ? "rgba(240,90,34,0.08)" : "rgba(63,141,255,0.08)",
      }}>{isMetric ? "Metric" : "BB"}</span>
    </div>
  );
};
