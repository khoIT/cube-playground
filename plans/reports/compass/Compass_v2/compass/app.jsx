/* global React, ReactDOM, TweaksPanel, TweakSection, TweakRadio, TweakToggle, useTweaks,
   ...
   CATALOG_BY_ID, METRIC_BY_ID, CONCEPT_BY_ID */
...
function CompassApp() {
  ...
  const [changeAnalysisConcept, setChangeAnalysisConcept] = useStateApp("m.revenue");
...
function Router({ tweaks, sidebarCollapsed, setSidebarCollapsed, openSubscribe, openSaveView, openChangeAnalysis }) {
  ...
  switch (route.name) {
    case "catalog":
      ...
      break;
    case "metric": {
      const c = CATALOG_BY_ID[route.id];
      breadcrumbs = [
        { label: "Catalog", onClick: () => go({ name: "catalog" }) },
        { label: "Metrics", onClick: () => go({ name: "catalog" }) },
        { label: c?.label || "Metric" },
      ];
      body = <MetricDetailPage
        id={route.id} layer="metric" tweaks={tweaks}
    ...
    case "data-model": {
      ...
      breadcrumbs = [
        { label: "Catalog", onClick: () => go({ name: "catalog" }) },
        { label: "Data Model", onClick: () => go({ name: "catalog" }) },
        { label: c?.label || "Building block" },
      ];
      body = <MetricDetailPage
        id={route.id} layer="data" tweaks={tweaks}
        openSubscribe={() => openSubscribe(c)}
        openSaveView={openSaveView}
        openChangeAnalysis={() => openChangeAnalysis(route.id)}
      />;
    ...
    case "explore":
      ...
      body = <ExplorePage
        ...
        tweaks={tweaks}
        openSaveView={openSaveView}
        openChangeAnalysis={() => openChangeAnalysis(route.measureId || "m.revenue")}
      />;
      break;
    case "wizard":
      breadcrumbs = [
        { label: "Catalog", onClick: () => go({ name: "catalog" }) },
        { label: route.layer === "data" ? "New building block" : "New metric" },
      ];
      body = <WizardPage layer={route.layer || "metric"} />;
      break;
    case "views":
