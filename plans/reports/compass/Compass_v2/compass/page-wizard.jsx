...
const WizardPage = ({ layer = "metric" }) => {
  const { go } = useNav();
  const toast = useToast();
  const [step, setStep] = useStateW(0);
  const [type, setType] = useStateW(layer === "metric" ? "derived" : "measure");
  ...
  const onPublish = () => {
    toast?.(layer === "data" ? "Building block published — YAML + metadata written" : "Metric published to the registry", { icon: "rocket" });
    go({ name: "metric", id: "m.revenue" });
  };
  ...
  return (
    <div style={{ maxWidth: 980, margin: "0 auto", padding: "24px 28px 60px" }}>
      <div style={{ marginBottom: 18, display: "flex", alignItems: "flex-start", gap: 14 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
            <h1 style={{ margin: 0, fontFamily: "var(--font-sans)", fontWeight: 600, fontSize: 22, color: "var(--neutral-950)", letterSpacing: "-0.02em" }}>{layer === "data" ? "New building block" : "New metric"}</h1>
            <span style={{
              padding: "2px 8px", borderRadius: 4, fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em",
              color: layer === "metric" ? "var(--layer-metric-accent)" : "var(--layer-data-accent)",
              background: layer === "metric" ? "rgba(240,90,34,0.08)" : "rgba(63,141,255,0.08)",
            }}>{layer === "metric" ? "Metric layer" : "Data Model layer"}</span>
          </div>
          <div style={{ fontSize: 13, color: "var(--neutral-600)" }}>
            {layer === "metric"
              ? "Author a named business metric. Writes to the metric registry — formula composes other metrics and/or building blocks."
              : "Author a cube primitive (measure, dimension, or segment). Writes Cube YAML and the catalog metadata atomically."}
          </div>
        </div>
      </div>
      ...
      <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: 12, padding: 28, minHeight: 380, marginTop: 18 }}>
        {step === 0 && <TypeStep type={type} setType={setType} layer={layer} />}
...
const METRIC_TYPES = [
  { value: "derived",       icon: "function-square", title: "Derived",            blurb: "Formula composes other metrics (e.g. ARPPU = Revenue / Paying Users)." },
  { value: "composed",      icon: "blocks",          title: "Composed",           blurb: "Picks a building block + time grain + cohort filter (e.g. WAU, MPU)." },
  { value: "parameterised", icon: "square-stack",    title: "Parameterised family", blurb: "An n-grid (A(n), PU(n), LTV(n))." },
];

const TypeStep = ({ type, setType, layer }) => {
  const options = layer === "data" ? WIZARD_TYPES : METRIC_TYPES;
  const heading = layer === "data" ? "What kind of building block?" : "What kind of metric?";
  const help = layer === "data"
    ? "Compass authors three kinds of cube primitive. They share the same metadata shape but route through different steps next."
    : "All three write to the metric registry. The middle steps adapt to the type you pick.";
  return (
    <div>
      <H2>{heading}</H2>
      <Help>{help}</Help>
        ...
        {options.map(t => (
          <div key={t.value} onClick={() => setType(t.value)} style={{
            ...
            {layer === "data"
              ...
              : <span style={{ width: 26, height: 26, borderRadius: 6, background: "var(--neutral-100)", color: "var(--neutral-700)", display: "inline-flex", alignItems: "center", justifyContent: "center" }}><Icon name={t.icon} size={14} /></span>}
      ...
      <div style={{ marginTop: 22, padding: 12, background: "var(--blue-50)", border: "1px solid var(--blue-200)", borderRadius: 8, display: "flex", gap: 10, alignItems: "flex-start" }}>
        ...
        <div style={{ fontSize: 13, color: "#1e40af", lineHeight: 1.5 }}>
          <b>Starting from GDS-1.8?</b> If you're authoring a canonical reference (e.g. <i>Paying Users</i>), Compass pre-fills the description and formula. You can override and Compass will flag your version as <i>drifted</i>.
        </div>
      </div>
    </div>
  );
...
const SourceStep = ({ type, draft, setDraft }) => (
  <div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginTop: 16 }}>
      {["mf_users", "active_daily", "user_recharge_daily", "recharge"].map(c => (
