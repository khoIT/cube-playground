...
- **Apollo is one consumer** (campaign delivery). It is currently blocked on CDP for richer predicates — CP unblocks it.
...
## 2. Why CP Is Uniquely Positioned

### 2.1 Predicate-vocabulary asymmetry
...
## 3. The Auto-Workflow Engine — Four Entry Points
...
## 4. Use Case Spotlights (Four Consumers)

### 4.1 Apollo — Campaign / Messaging / Channel Delivery
...
### 4.2 Customer Service — VIP-at-Risk Call List (the use case you named)
...
### 4.3 Payment / VNGGames Pay — Promo Eligibility & Refund-Risk Gating
...
### 4.4 External Ad Platforms — Meta / TikTok Past-Segment Uploads
...
## 5. The Universal Publish Contract (one shape, four consumers)
...
### 5.1 Segment record (canonical, returned by `GET /segments/:id`)
...
### 5.3 Identity translation at the edge
...
### 5.4 Why one contract beats N bespoke
...
**Estimate (rough, no commitment):** ~80% of the platform is built. The new pieces are intentionally small — a publish endpoint, a check endpoint, a subscriber table, and one bridge from anomaly to segment.

---

## 7. What Each Consumer Team Has to Do

| Team | To consume CP segments, you need to: | Effort |
|---|---|---|
| **Apollo** | Build `POST /apollo/v1/segments` to ingest external segments OR subscribe via webhook to CP's publisher; surface them as "Source: cube-playground" in your UI | ~1–2 weeks based on the LU Web Discovery Report estimate |
...
## 8. Risks & Trade-offs (Honest)

1. **PII / data residency.** Pushing UID lists to CS means CS receives phone numbers. Pushing to Meta means hashing advertising IDs. CP today holds *neither* — it has cube-native dimensions. The identity-translation layer (§5.3) must (a) be auditable, (b) never leak phone/email *into* CP, (c) emit hashed identifiers where the consumer requires. Probably a one-way resolver call to a Pay/CRM-owned identity service — **CP never stores phone**.
...
## 9. What I'd Recommend (Concrete Asks)

1. **Endorse the framing** — CP as segment-of-record, Apollo as one consumer of N. This unlocks investing in the publish primitives instead of the Apollo-specific seam.
2. **Greenlight the 3 publish primitives** as the cross-team contract. Document and circulate. Three small endpoints + one subscriber table.
...
4. **Hold a 1-hour cross-team review** (Growth + Apollo + Payment + CS leads) using this report as the pre-read. Goal: collect each team's required `identity_system` + which of the three verbs they want.
