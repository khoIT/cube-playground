...
| 03 | [Memory settings panel + chat header chip](./phase-03-memory-settings-panel.md) | P1 | **Done** — GET/DELETE `/api/chat/sessions/:id/focus` + SSE `focus_updated`/`focus_reset` events + `useSessionFocus` hook + `ChatHeaderFocusChip` (chat thread + side-panel) + Settings → Chat memory section + `/forget` slash command + 10 tests | `CHAT_MEMORY_UI` |
| 04 | [Cancellation, timeouts, error UX](./phase-04-robustness-cancel-timeout.md) | P1 | **Done** — backend (registry abort + runner signal + cancel endpoint + timeout + turn_aborted SSE) + FE (`useCancelTurn` hook, `TurnCancelButton`, FE SSE union extended with `turn_started`/`turn_aborted`, store reducer surfaces `turnId` + `aborted` status, button mounted on chat-thread page + side panel) + 11 tests total | `CHAT_TURN_TIMEOUT_MS` |
| 05 | [Observability unification](./phase-05-observability-unification.md) | P1 | **Partial — infra landed** — `TurnTracer` + `TraceEvent` discriminated union + `StructuredLogSink` + `ObserverSinkAdapter` + 10 tests. Parallel-emit shim + claude-runner cutover + production soak deferred to a follow-up session (gated on 7-day diff-zero comparison) | — |
...
| 08 | [Business-metric audit trail](./phase-08-business-metric-audit.md) | P3 | **Done** — backend (migration 016 + audit store + POST/PATCH instrumented + /history route + chat tool + 10 tests) + FE History tab (replaces `tab-activity` placeholder, `useBusinessMetricHistory` hook, action pill + actor + reason renderer, 4 tests) | — |
| 09 | [Test coverage uplift](./phase-09-test-coverage.md) | P2 | **Partial — unit tests landed organically (710 tests across 89 files); eval harness + nightly CI + `@vitest/coverage-v8` tooling deferred to follow-up session** | — |
