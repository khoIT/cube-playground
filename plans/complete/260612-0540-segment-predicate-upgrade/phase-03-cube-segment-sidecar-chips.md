...
- **[RED-TEAM major — PATCH interplay spec]** Define precedence explicitly in the handler (and test each): (a) both `predicate_tree` + `cube_segments` present → filters from tree, segments from patch (no carry-forward); (b) only `cube_segments` → rebuild `cube_query_json` from STORED tree's filters + new segments; reject 400 if no stored tree (nothing to scope); (c) only `predicate_tree` → today's carry-forward; (d) refresh enqueues when EITHER changed; (e) `cube_segments` joins `visibility`/`predicate_tree`/`uid_list`/`type` in the administer-gated field list. Canonical-sort segments before persisting; equality check against stored value to avoid no-op refreshes (supersedes the byte-identical phrasing above).
- **[RED-TEAM major — cross-cube sidecar]** Sidecar may carry OTHER cubes' segments (e.g. `mf_users.whales` on an active_daily segment). Chips for non-primary-cube entries render read-only (visible, labeled with their cube, not toggleable) and are ALWAYS preserved in the saved set — only primary-cube segments are editable this phase.
