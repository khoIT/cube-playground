...
- **W3/W4 measure backfill — ✅ DONE (template upgrade).** Resolved the mechanism question: the std monthly source tables are **uniform across all 8 games** (std_ingame_user_recharge_monthly=55 cols, std_ingame_user_active_monthly=43 cols) → all oracle measures are portable, none source-blocked. Upgraded the shared cfm template (`cfm/{user_recharge_monthly,user_active_monthly}.yml`) to a SUPERSET — added the genuinely-new oracle profile measures/dims (first/last/min/max recharge & active role_level/vip_level/fighting_power, IAP/web/USD splits, recharge_days, first/last profile attrs), kept all dev measures, **skipped ~9 oracle names that are exact dev-renamed duplicates** (`row_count`≈`rows`, `ingame_total_recharge_value_vnd`≈`revenue_vnd_total`, `active_days`≈`active_days_total`, etc.) to avoid redundancy. Propagated via `onboard-game-cube-model.mjs <g> --force --only user_recharge_monthly,user_active_monthly` to **only the 4 gap games (ballistar, ptg, pubg, jus)** — cros/tf/muaw deliberately untouched (cros is hand-augmented; force would clobber it). Verified: 10 cube files changed (2 template + 8 regen), +1260/-0 (purely additive), harness parity **121→40 (−81)**, 0 correctness, 0 parse errors, no unscoped cube touched. Remaining 40 parity = name-alias dupes (intentional) + non-monthly gaps.
- **W6 metric-layer FIXABLE GAPs — partially addressed** by the above (online-time, recharge profile measures now present); remaining metric GAPs (gacha/tutorial/funnel) are source-blocked → W7 N/A.
