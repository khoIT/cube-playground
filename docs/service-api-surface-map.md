...
| POST | `/api/segments/translate-query` | none | `{query: CubeQuery}` | segmentability probe for the chat "Build segment from this" bridge: `{segmentable:true, predicate_tree, cube}` or `{segmentable:false, reason, hint, seed_dimensions?, cube?}`. Pure, no Trino. Gate mirrors chat-service translator (rejects measure_filter / time_leaf_in_or / order_limit_without_measure; filterless → `no_predicate`, or `breakdown_unfiltered`+`seed_dimensions` when grouping dims exist — NEVER a silent match-all) | cube-query-to-predicate (server mirror of chat-service util) |
| POST | `/api/segments/dimension-values` | none | `{game_id, dimension, query: CubeQuery}` | distinct values of a grouping dimension, for the breakdown seed picker (the `breakdown_unfiltered` path): `{values:string[], approx:false, took_ms}`. Runs the explored breakdown query as-is (measures `.slice(0,1)`, limit ≤100), dedups the dimension column, skips nulls. Best-effort — Cube failure → `{values:[], reason, took_ms}` (HTTP 200, picker degrades) | loadWithContinueWait + resolveCubeTokenForGame |
| POST | `/api/distribution` | none | `{game_id, member, population_predicate?, buckets?=10}` | decile histogram of a per-user measure: `{buckets:[{lo,hi,count}], total, p50, p90, approx:true, took_ms}`; slow/failed/non-catalog → `{buckets:null, reason, took_ms}` (HTTP 200, picker falls back to numeric input) | measure-distribution (segmentable-measures catalog allowlist → approx_percentile via runQuery) |
| POST | `/api/profile` | none | `{game_id, cube, predicate, dimensions?}` | pre-save cohort profile: `{total, breakdowns:[{dimension,label,top:[{value,count,pct}]}], approx, took_ms}`; per-dim best-effort (Promise.allSettled), missing dims omitted, total failure → `{total:null, breakdowns:[]}` | cohort-profile-runner (member360 panel dims → Cube `/load` group-by) |
| POST | `/api/overlap-candidate` | own segments only | `x-owner` `{game_id, cube, predicate}` | novelty guard: `{overlaps:[{segment_id,name,candidate_size,both_count,pct_of_candidate}], approx:true, took_ms}` (top-3, ≥25%); APPROXIMATE — sampled candidate uids ∩ snapshot membership. Empty/error → `[]` (never 500). Scoped to caller's OWN same-game segments | candidate-overlap-counts (Cube uid sample ∩ SEGMENT_MEMBERSHIP_DAILY) |
...
| POST | `/api/segments` | editor, admin | `authorization`, `x-owner` | 201 segment (visibility=personal default; `born_from` lineage persisted when crystallized from an exploration) | SQLite, translator, refresh-queue |
...
| GET | `/api/segments/:id/cs-care` | none | `?scope=(all\|paying)` | `{coverage{totalMembers,contactedMembers,pct,truncated},freshness{csMaxLogDate},pulse{tickets,contacted,openUnresolved,negativeSentiment,lowRating},issueMix[{category,tickets,members}],watchlist[{uid,name,ltv,lastCategory,lastSource,sentiment,rating,statusGroup,daysSince,riskScore}],csImpact{contacted,nonContacted,windowDays,smallSample}\|null}` gated predicate+CS-game only; 404 NO_CS_CARE; 6h TTL; recharge-fail degrades csImpact→null (200); CS-read-fail → 502. **Names:** watchlist members missing a stored name (rank below profile cap) resolved live; resolved names cached with 6h payload. **`scope=paying`:** payer sub-cohort (predicate ∩ `paying_lifetime`, ranked, cap 5000) resolved LIVE; separate process-local cache (no durable-cache collision). | lakehouse/cs-product-map, lakehouse/cs-ticket-reader, lakehouse/cs-recharge-trajectory, segment-cs-care-assembly, resolve-member-names-live, segment-cohort-context |
| GET | `/api/segments/:id/member-tiers` | none | `?scope=paying` (required) | `{tiers: MemberTiers\|null}` live "paying users only" top/middle/bottom-50 (default tiers ship inline on detail payload); 400 UNSUPPORTED_SCOPE for non-paying; `tiers:null` when sub-scope N/A (non-mf_users hub / no rank measure); 502 PAYING_TIERS_UNAVAILABLE; 10-min process cache (never precomputed). | segment-paying-tiers, segment-cohort-context, member-tier-runner |
| GET | `/api/segments/:id/members/:uid/cs-tickets` | none | — | `{segmentId,gameId,productId,uid,member:{name,ltv},coverage:{joined,note},freshness:{csMaxLogDate},recharge:CohortRechargeStats&{windowDays}\|null,tickets:CsTicketDetail[]}` gated guardSegment(read) + uid∈uid_list_json (404 NOT_IN_SEGMENT); predicate segment + CS-covered game only (404 NO_CS_CARE); 6h TTL keyed(segmentId,uid); MAX_TICKETS=60, MAX_MESSAGES_PER_TICKET=80, MAX_RATINGS_PER_TICKET=10, LOOKBACK_DAYS=365; CS-read fail→502 CS_TICKETS_UNAVAILABLE; recharge-fail→null (200). **Names:** member.name resolved live if missing from snapshot; cached with 6h payload. | lakehouse/cs-ticket-detail-types, lakehouse/cs-ticket-detail-reader, lakehouse/cs-ticket-detail-signals, lakehouse/cs-recharge-trajectory, resolve-member-names-live |
