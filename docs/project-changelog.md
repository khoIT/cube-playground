...
## 2026-06-18 — Segment snapshots: per-user state + KPI time-series + movement read API

Extends segment-membership lakehouse from "daily uid-only" to per-segment-cadence, multi-dimensional capture + tokenless movement analytics.

- **Configurable capture cadence** (migration 063): each segment carries `snapshot_cadence` (15m|1h|3h|6h|12h|default daily). Job tick 15m (`snapshot-segment-membership.ts`); per-segment elapsed-check (cheap, SQLite); only cadence-due segments run Trino. `snapshot_ts` is the canonical `YYYY-MM-DD HH:MM:00` bucket key (GMT+7), threaded through all snapshot tables (migration 064: additive `snapshot_ts` on membership/definition/delta).
- **Per-user state snapshots** (segment_member_state_daily): per (snapshot_ts, segment, uid) row carries mf_users canonical state (ingame_name, ltv_vnd, ltv_30d_vnd, is_paying_*, total_active_days, days_since_last_active, …). Schema from `CANONICAL_USER_STATE_COLUMNS` (service/snapshot-cadence.ts + lakehouse/canonical-metric-set.ts). Writer: predicate-free mf_users projection (Cube `/sql`, cached per game+ts) JOINed in Trino to membership@ts (scan-wide, write-narrow; avoids per-segment join-rooting). Columns absent from a game's /meta dropped to NULL.
- **KPI time-series** (segment_kpi_daily): per (snapshot_ts, segment, metric) row = `runScopedKpi` (card-runner, same path as Insights tab). Non-additive measures (paying_rate_30d, arppu_vnd) correct because Cube-computed, never derived from per-user state. Empty cohort → NULL value (not omitted, so time-series is complete). Idempotent per (game, segment, snapshot_ts) via DELETE+INSERT.
- **Movement read API** (segment-movement.ts): tokenless, stale-safe, redaction-parity with members API. Four endpoints: (1) KPI-trend (per metric over time), (2) movement-series (entry/exit), (3) state-distribution (histogram of a dimension at a snapshot), (4) state-distribution-trend (histogram over time). Request-time downsampling via downsample-snapshots.ts (last-in-bucket, never sum; mixed-cadence collapses to coarser, steps toward finer carry-forward). Effective_granularity + cadence-change detection in response.
- **Smoke-test gate before prod deploy** (lessons-learned.md): state writer assumes Cube `/sql` aliases as `cube__field` (regex-parsed offline). If alias format drifts, INSERT lands zero rows silently. Before shipping, run writer on live segment + `SELECT COUNT(*)` from segment_member_state_daily — must be > 0.

## 2026-06-16 — Query Performance & Optimization Hub — admin query telemetry + rollup-suggestion engine

Live Cube query monitoring console at `/admin/query-perf`. Gateway `/load` handler captures latency/status/rollup routing for every request (200-sampling, 30d retention). Admin UI: KPI strip (total/failures/p95/fallthrough/slow) → failures triage table → master-detail Optimize panel. Suggestions via deterministic classifier→playbook-matching (always available) + optional on-demand LLM analysis (per-admin rate-limit, per-id cache, hard timeout, sonnet-pinned; gated on `needsLlm`, 409 when a playbook already fits). Pre-agg YAML scaffolder generates drafts from query shape. Routes: `GET /api/query-perf/{failures,recent,summary}`, `GET /api/query-perf/:id/{suggestion,scaffold}`, `POST /api/query-perf/:id/llm-suggest`. Service layer: store (fire-and-forget to SQLite), classifier (tri-state verdict), playbooks + scaffolder (deterministic), LLM suggester (on-demand cached). Background job prunes rows >30 days old. Recommendations render as an **inline expandable row** (click a failure → expands verdict + remedy + draft YAML in place; no separate Optimize action button yet). A **"Used in" column** shows which surface issued each query (query-builder / dashboard / segment+tab / chat conversation) via an explicit `x-cube-source` header set by both client transports + chat-service. The expanded panel also shows the **verbatim query** — the admin `query_perf` table stores the full query incl. values/dateRange (`query_full`, migration 062), a deliberate admin-only posture bounded by admin routes + 30d retention; the activity spine stays names-only.
...
- **Categorical cluster palette** (`tokens.css`) — new purpose-built 9-hue `--cluster-*` scale (light + dark pairs). Replaces the semantic/layer tokens that collapsed into 3 blues + 3 ambers, so clusters are now mutually distinguishable. Single source of `{label, accent}` in `cube-graph/cube-clusters.ts` (DRY across nodes / cluster boxes / legend / edges); cluster boxes show friendly labels.
- **Legend + stats** (`cube-graph-legend.tsx`, toolbar) — mirrors `concept-map-legend`; renders **only present clusters** (no overflow). Added `N cubes · M joins · K views` stats line + interaction hint.
- **Colored, directional edges** (`cube-graph-board.tsx`) — edges take their source cluster's color + `ArrowClosed` arrowheads (CSS-var marker fills resolve + theme-react). Selection still highlights incident joins in brand and dims the rest.
- **Tabbed cube drawer** (`detail-panel.tsx` + new `detail-panel-members.tsx`) — accent header, clamped description, Joins section (colored target · relationship · `col → col` key label), then **Dimensions / Measures / Segments** tabs. Every member links into the catalog (measures → metric card, dims/segments → concept page; PK/private dims non-clickable to avoid dead links). **Removed "Open in Playground"** (the member links are the canonical query entry). Folded the old `detail-panel-measures` into the members tab (reuses `MeasureRow`).

## 2026-06-12 — Cube join-graph as default Data Model view (IA reorder + Graph/grid toggle)

Redesigned the Data Model landing as an interactive join-graph canvas (Graph-default, Grid-toggle view). Reordered IA: **Cubes, Schema, Concepts, Models, Concept Map** (Schema no longer first). Schema Cartographer moved to `/catalog/data-model/schema` sub-path. Cube graph builder pure-TS (extended-meta join parser, cluster-grid layout, view→cube mapper, `cubeBaseName` prefix-stripping for cross-workspace equivalence). Route matrix: root `/catalog/data-model` → Cubes (Graph), `/catalog/data-model/schema` ← legacy root, `?focus=` preserved for chat deeplinks. Lazy load (~11kB reactflow chunk, reuses concept-map shared bundle). Tests: full Catalog suite green; build clean.

- **Cubes surface** (`cubes-surface.tsx`) — Tab owner, Graph ⇄ Grid toggle, URL-derived state (`?view=grid`), `history.replace` for cleaner history, shared toggle controls reused by other tabs.
- **Cube-graph module** (`src/pages/Catalog/cube-graph/`) — Pure-TS join-graph builder (`build-join-graph.ts` parses extended-meta `joins[].sql` `${ref}.col` tokens, maps belongsTo/hasMany/hasOne → N:1/1:N/1:1, unordered-pair edge dedup, isolated + missing-target lints). Deterministic cluster-grid layout (`cluster-grid-layout.ts`, TS port of model-viewer/gen_layouts.py anchors). View→cube mapper (`view-composition.ts` reads `aliasMember` from extended-meta). `cube-graph-page.tsx` (lazy-loaded).
- **IA route refactor** (`catalog-page.tsx`) — Redirect matrix: root + `?focus=` → Schema (preserves focus for search+chat); `/catalog/schema?focus` → `/catalog/data-model/schema` (chainable). Root `/catalog/data-model` chains Cubes, Schema chains to new path. `CatalogBrowseBody` extracted (refactored, verbatim).
- **Extended-meta** — `use-catalog-meta.ts` additive `aliasMember?: string` on CatalogMeasure/CatalogDimension (powers view-composition).

## 2026-06-12 — Segment predicate explorer + playground round-trip (identity-anchor pivot + meta-driven picker + sidecar chips + definition deeplink + save-back)

Closes the build↔segment loop: refine a segment's definition in the Playground and save it back to the segment editor. Preset-less cubes now pivot to mf_users, member fields are discoverable via Cube meta, and the editor fully supports in-place predicate authoring. Five subsystems ship as one: (1) identity-anchor pivot sweep seeds 8 cubes with mf_users roots; (2) meta-driven member picker feeds Cube's `/meta` connectedComponent reachability; (3) cube-segment sidecar scope chips allow owner/admin to toggle model-defined segments; (4) definition deeplink converts segment predicate trees to Cube filters preserving relative-date literals; (5) save-back patches the predicate via PATCH and auto-refreshes. Plan: `plans/260612-0540-segment-predicate-upgrade/`. Tests: 13-row fidelity matrix recorded (12 pass, 1 documented partial — intra-session cube switch doesn't drop edit mode); server route/translator suites green; FE predicate-builder/playground/save-bar suites green; code-review round (2 critical, 7 major) fully burned down same-day.

- **Identity-anchor pivot sweep** — cube_identity_map now has 8 rows (active_daily, ordered_event_funnel, ordered_funnel_canonical, user_active_rolling, user_gameplay_daily, user_roles, user_recharge_daily, user_recharge_rolling → mf_users.user_id). Cross-game collision audit clean (jus_vn recharge excluded by design). Preset-less cubes render "via mf_users" chip + enriched Members tab.
- **Meta-driven member picker** (`use-predicate-member-catalog.ts`) — Searchable Select fed by `/meta` connectedComponent reachability; auto-sets leaf type. On-focus distinct-value suggestions (use-dim-value-suggestions.ts, cached per workspace/game/dim, 50-row max). Degrades to free-text when meta unavailable. Replaces hardcoded Input in predicate-leaf.tsx.
- **Cube-segment sidecar chips** (cube-segment-scope-chips.tsx) — Toggleable scope toggles in editor; model-defined segments render read-only + always-preserved when cross-cube. Server PATCH accepts cube_segments with precedence spec (tree+segments / segments-only / tree-only carry-forward), canonical sort, equality no-op guard.
- **Definition deeplink** (predicate-tree-to-cube-query.ts) — Segment tree → Cube filters preserving relative-date literals (inDateRange, beforeDate, afterDate) + boolean AND/OR groups. Identity dim + sidecar auto-attached. Replaces broken `?from-segment=` (uid-IN only, >8k-char UIDs). Deeplink consumer migrated from saved-analyses-tab.tsx (uid-IN overlay stays via new deeplink).
- **Save-back** — Playground "Refining: <segment>" banner; SegmentsSaveBar "Update <name>" action (translatability gate blocks empty-predicate + nested-group constructs). Echo filters stripped by exact structural match. PATCH enqueues an immediate refresh (same path as predicate edits). Can_administer-gated. Conflicts: last-write-wins.
- **Server** — routes/segments.ts (withCubeSegments reuse + cube_segments PATCH precedence), treeToCubeFilters (tree→filters), buildPredicateFromRows (query→tree). Identity-map rows seeded via PUT /api/identity-map (no migration — data, not schema). cube_query_json holds expanded date literals, which is why deeplinks source the predicate tree.
- **FE** — src/pages/Segments/editor (member picker, sidecar chips), src/QueryBuilderV2/playground (deeplink boot), segments-save-bar (update mode). Core tree↔query bridges: predicate-tree-to-cube-query.ts + build-predicate-from-rows.ts.
