...
- `src/pages/Liveops/` — **LiveOps monitoring center** (phases 01–07, 2026-06-24). Five-section hub: **Command Center** (`/liveops`) — KPI strip + high-severity anomaly strip + game-scoped trends + cross-game portfolio grid (when "All games" selected). **Diagnostics** (`/liveops/diagnostics`) — three-tab sub-hub: Delta decomposition (WoW/MoM contribution waterfall), Event timeline (annotation overlay), Lifecycle flow (state cards + Sankey). **Monetization** (`/liveops/monetization`) — payer-tier distribution, LTV-by-cohort, revenue concentration (Gini), SKU/pack performance. **Retention** (`/liveops/retention`) — cohort grid (alias `/liveops/cohort`). **Alerts & Digests** (`/liveops/alerts`) — anomaly inbox + alert-rules engine + scheduled digest subscriptions (alias `/liveops/anomalies`). Reuses `KpiTile`, `Sparkline`, `LiveBadge`, `assistant-chart-section.tsx`. New backends: `server/routes/{lifecycle-flow,monetization-deepdive,annotations,alert-rules}.ts`, `chat-service POST /liveops/delta-decompose`, cross-service notification bridge via `POST /internal/notifications`.
